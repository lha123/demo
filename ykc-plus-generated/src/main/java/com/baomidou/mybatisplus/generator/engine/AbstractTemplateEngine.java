...
public abstract class AbstractTemplateEngine {
    ...
    protected void outputCustomFile(@NotNull List<CustomFile> customFiles, @NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        ...
        customFiles.forEach(file -> {
            ...
            filePath = getConfigBuilder().getGlobalConfig().getOutputDir()+File.separator+filePath.replaceAll("\\.", StringPool.BACK_SLASH + File.separator);
            ...
            outputFile(new File(fileName), objectMap, file.getTemplatePath(), file.isFileOverride(),controllerd.isNotCreate());
        });
    }
     ...
     * @since 3.5.0
     */
    protected void outputRepo(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        String entityName = tableInfo.getEntityName();
        String repoPath = getPathInfo(OutputFile.repo);
        if (StringUtils.isNotBlank(tableInfo.getRepoName()) && StringUtils.isNotBlank(repoPath)) {
            getTemplateFilePath(TemplateConfig::getRepo).ifPresent(repo -> {
                String repoFile = String.format((repoPath + File.separator + tableInfo.getRepoName() + suffixJavaOrKt()), entityName);
                outputFile(new File(repoFile), objectMap, repo, getConfigBuilder().getStrategyConfig().repo().isFileOverride());
            });
        }
        String repoImplPath = getPathInfo(OutputFile.repoImpl);
        if (StringUtils.isNotBlank(tableInfo.getRepoImplName()) && StringUtils.isNotBlank(repoImplPath)) {
            getTemplateFilePath(TemplateConfig::getRepoImpl).ifPresent(repoImpl -> {
                String repoImplFile = String.format((repoImplPath + File.separator + tableInfo.getRepoImplName() + suffixJavaOrKt()), entityName);
                outputFile(new File(repoImplFile), objectMap, repoImpl, getConfigBuilder().getStrategyConfig().repo().isFileOverride());
            });
        }
    }
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            tableInfoList.forEach(tableInfo -> {
                ...
                // repo
                outputRepo(tableInfo, objectMap);
                // service
...
//    }
    ...
    public Map<String, Object> getObjectMap(@NotNull ConfigBuilder config, @NotNull TableInfo tableInfo) {
        ...
        Map<String, Object> repoData = strategyConfig.repo().renderData(tableInfo);
        objectMap.putAll(repoData);
